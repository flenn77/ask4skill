// src/pages/coach/GamesManage.jsx
...
export default function GamesManage() {
  ...
  const load = async () => {
    try { setLoading(true); setErr("");
      const { data } = await api.get("/coach/games");
      setItems(data || []);
  ...
  const add = async () => {
    if (!val.trim()) return;
    try { await api.post("/coach/games", { game: val.trim() }); setVal(""); load(); }
    catch(e){ setErr(e.response?.data?.error || "Erreur ajout"); }
  };
  const del = async (game) => {
    try { await api.delete(`/coach/games/${encodeURIComponent(game)}`); load(); }
  ...
  return (
    <Paper sx={{ p:2, borderRadius:2, border:"1px solid rgba(124,58,237,.22)", bgcolor:"rgba(13,17,28,.6)" }}>
      <Stack spacing={1.25}>
        <Typography variant="h6" sx={{ fontWeight:900 }}>Mes jeux</Typography>
        {err && <Alert severity="error">{err}</Alert>}

        <Stack direction="row" spacing={1}>
          <NeonField label="Nouveau jeu (ex: Valorant)" value={val} onChange={e=>setVal(e.target.value)} />
        ...
        {loading ? <CircularProgress/> : (
          <Stack direction="row" spacing={1} flexWrap="wrap">
            {items.map(i => (
              <Chip key={i.id} label={i.game} onDelete={()=>del(i.game)} />
            ))}
            {!items.length && <Typography sx={{ opacity:.7 }}>Aucun jeu.</Typography>}
          </Stack>
        )}
      </Stack>
    </Paper>
